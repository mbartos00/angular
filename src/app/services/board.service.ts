...
import { Board, TBoard } from '../components/board/Board';
...
export class boardService {
  ...
  getBoard(id: number): Board {
    return new Board(
      this.getBoardList().find((board: TBoard) => board._id == id)
    );
  }

  getAllBoards(): Board[] {
    const boards: Board[] = this.getBoardList().map(
      (obj: TBoard) => new Board(obj)
    );
  ...
  editBoard(id: number, newTitle: string) {
    const boards = this.getBoardList();
    const index = boards.findIndex((board: TBoard) => board._id == id);
