...
export class TaskListService {
  getCurrentId(boardId: number) {
    ...
    return !!taskLists && taskLists.length
      ? taskLists.sort((a, b) => b.id - a.id)[0].id
      : 0;
  }
  ...
  removeTaskList(boardId: number, taskListId: number) {
    ...
    this.setTaskLists(
      boardId,
      taskLists.filter((taskList) => taskList.id !== taskListId)
    );
  }

  setTaskLists(boardId: number, taskLists: TaskList[]) {
    window.localStorage.setItem(
      TASK_LIST_KEY + boardId,
  ...
  private loadTaskLists(boardId: number): TaskList[] {
    let taskLists = JSON.parse(
      window.localStorage.getItem(TASK_LIST_KEY + boardId) || '[]'
    );
